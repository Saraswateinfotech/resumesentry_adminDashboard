...
const ShowResumeRS = ({ row, allData }) => {
  ...
  const [currentIndex, setCurrentIndex] = useState(
    allData.findIndex((item) => item.submission_id === row.submission_id),
  );
        ...
        try {
          const response = await fetch(
            "https://apis.resumesentry.com/resumes/updateResumeStatus",
            {
              ...
              body: JSON.stringify({
                // resume_id: allData[currentIndex]?.resume_id,
                submission_id: allData[currentIndex]?.submission_id,
                status: newStatus,
              }),
            }
          );
...
"
      ...
      <div className="bothbtn mt-5 border-t border-gray-200 pt-5 text-center">
      <button
          type="button"
